...
export default function WorkspaceSwitcher() {
    ...
    useEffect(() => {
        if (workspaces.length > 0 && !WorkspaceStorage.get()) {
            WorkspaceStorage.set(workspaces[0].id)
        }
    }, [workspaces])
                        ...
                        <div className="absolute top-full left-0 mt-2 w-56 bg-navy-800 border border-white/[0.07] rounded-xl shadow-card z-50 overflow-hidden animate-fade-in">
                            ...
                            <div className="border-t border-white/[0.05] p-1">
                                <button
                                    ...
                                    className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/[0.04] text-sm text-slate-400 hover:text-white transition-colors"
                                >
                                    <Plus size={13} />
                                    Create workspace
                                </button>
